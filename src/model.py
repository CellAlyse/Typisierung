...
def conv_bn(
    ...
    """
    Das ist eine benutzerdefinierte Konvolutionsfunktion:
    :param filters --> Anzahl der Filter für jede Konvolution
    :param kernel --> die Kernelsize
    :param activation --> die allgemeine Aktivierungsfunktion (relu)
    :param strides --> Anzahl der strides
    :param padding --> Modellpadding (kann valid oder same sein)
    :param type --> um anzugeben, ob es sich um eine Transponierte oder normale Konvolution handelt

    :return --> gibt die Ausgabe nach der Konvolution und der Batchnormalisierung und Aktivierung zurück.
    """
...
def get_callbacks(name):
    """
    Dies ist eine benutzerdefinierte Funktion, um nur die beste Checkpoint zu speichern.
    :param name --> name
    """
    return [
        tf.keras.callbacks.ModelCheckpoint(
            f"models/{name}.h5", save_best_only=True, save_weights_only=True, verbose=1
        )
    ]
...
def mean_iou(y_true, y_pred):
    ...
    score = tf.map_fn(
        lambda x: iou(y_true_32, tf.cast(y_pred_32 > x, tf.float32)),
...
def do_unet():
    """
    Das ist das U-Net Modell.
    Es ist ein benutzerdefiniertes U-Net mit optimierter Anzahl an Layern.
    >> model.summary()
    """
    ...
    if cell_type == "rbc":
        model.compile(
            loss={"mask": tversky_loss, "edge": tversky_loss},
            loss_weights=[0.1, 0.9],
            optimizer=opt,
            metrics={
                "mask": [mean_iou, dsc, tversky],
